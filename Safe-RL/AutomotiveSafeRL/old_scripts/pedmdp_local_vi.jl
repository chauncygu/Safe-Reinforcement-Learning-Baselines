...
function sample_points(mdp::PedMDP, n_samples::Int64, rng::AbstractRNG)
...
function convert_states(mdp::PedMDP, sampled_states::Vector{Int64})
    n_routes = 4
    n_features = 4
    nd = n_features*3 + n_routes + 1
...
function set_terminal_costs!(mdp::PedMDP, knnfa::LocalNNFunctionApproximator)
    for (i, pt) in enumerate(knnfa.nnpoints)
        s = convert_s(PedMDPState, pt, mdp)
