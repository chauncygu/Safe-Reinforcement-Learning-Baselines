...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", condition="Condition1", smooth=1, **kwargs):
    ...
    #print(data.head())
    data.rename(columns={"EpCost":"Cost","EpRet":"Reward", "TotalEnvInteracts":"Steps"}, inplace=True)
    data.rename(columns={"AverageEpCost":"Cost","AverageEpRet":"Reward"}, inplace=True)
...
def get_datasets(logdir, condition=None):
    ...
    global units
    datasets = []
    for root, _, files in sorted(os.walk(logdir)):
        if 'progress.txt' in files:
            ...
            try:
                ...
                print('Read from %s'%os.path.join(root,'progress.txt'))
            except:
