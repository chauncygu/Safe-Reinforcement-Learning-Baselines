...
class Logger:
    ...
    def save_config(self, config):
        ...
        if proc_id()==0:
            ...
            print(output)
            with open(osp.join(self.output_dir, "config.yml"), 'w') as out:
                #out.write(output)
                yaml.dump(config, out, indent=4)
    ...
    def save_data(self, data, fname="data.pickle"):
        try:
            joblib.dump(data, osp.join(self.output_dir, fname))
        except:
            self.log('Warning: could not pickle the data you want to save!.', color='red')
