...
# ╔═╡ 9e08cb5b-f0d0-48aa-9901-356a46565d2f
md"""
# The No Recovery Area

Takes about 20 minutes to run. 
"""

# ╔═╡ 0253730c-3cb9-4489-af7c-dc907212f4fe
# Used for getting a local variable scope inside a cell.
call(f) = f()
...
begin
	...
	VERY_BAD_SQUARE = 3 # Certain safety violation
	nothing
end
...
# ╔═╡ bdb276f6-e369-4524-8ef0-590ea4b1ca29
begin
	mergedshieldlabels = 	["{hit, nohit}", "{hit}", "{}", "No Recovery"]
	mergedshieldcolors = 	[colorant"#ffffff", colorant"#a1eaff", colorant"#ff9178", colorant"#e74c3c"]
	(;mergedshieldcolors, mergedshieldlabels)
end

# ╔═╡ 53d29226-7839-4fa1-bc56-077d4355042f
@bind mechanics PlutoUI.combine() do Child
md"""
### Configure the regular mechanics

These would be the mechanics used in general for the bouncing ball problem. 

When a safe strategy is synthesized, it will automatically consider the worst-case outcome.
...
# ╔═╡ ad79105a-dc0f-4bef-ad2c-d331d6c385ac
@bind optimisticmechanics PlutoUI.combine() do Child
md"""
### Configure the optimistic mechanics

These would be the mechanics altered to only allow the best-case outcome, from the perspective of the player. 
If there are no safe actions under these mechanics, this also means that there is no chance of the ball recovering under the regular mechanics.

These values update automatically when the regular mechanics change.

`t_hit = ` $(Child("t_hit", NumberField(-100:0.01:100, default=0.10)))

`g = ` $(Child("g", NumberField(-100:0.01:100, default=-9.81)))

`β1 = ` $(Child("β1", NumberField(-100:0.01:100, default=mechanics.β1 + mechanics.ϵ1)))
`ϵ1 = ` $(Child("ϵ1", NumberField(-100:0.01:100, default=0.00)))

`β2  = ` $(Child("β2", NumberField(-100:0.01:100, default=mechanics.β2 + mechanics.ϵ2)))
`ϵ2  = ` $(Child("ϵ2", NumberField(-100:0.01:100, default=0.00)))
...
`G = ` $(Child(NumberField(0.01:0.01:1, default=(@nbparam "G" 0.02))))
...
md"""
`samples_per_axis = ` $(@bind samples_per_axis confirm(NumberField(1:30, default=(@nbparam "samples_per_axis" 3))))
"""
...
The transition function $R$ is defined using the function `get_reachable_area` from the file `BarbaricReachabilityFunction.jl`. The value `samples_per_axis` is the number of evely spaced samples for the v and p axes, that are used to estimate the set of squares that are reachable from an intial square.
"""

# ╔═╡ 88648d7a-1ac3-4f4d-903b-57ad8fdc5522
R_regular = get_barbaric_reachability_function(samples_per_axis, mechanics)

# ╔═╡ d6f57db2-2ad3-412c-9dff-81bacb719781
R_optimistic = get_barbaric_reachability_function(samples_per_axis, optimisticmechanics)

# ╔═╡ beb00a60-4939-41be-b04f-702e9389f23f
md"""
Number of samples taken per square at `samples_per_axis=`$(samples_per_axis):

$(length(grid_points(box(grid, 0, 0), samples_per_axis)))
"""

# ╔═╡ b1de6876-b41d-4b00-ba88-e504a65e07dc
# shield, _terminated_early_, _animation_
shield, _, _ = make_shield(R_regular, grid)
...
# ╔═╡ c3c1e889-43dd-4289-b2d5-a345a12b037f
if  get_value(box(shield, 0, 7)) == 2
	md"""
!!! warning

	Shield does not consider all initial states safe. The initial state (0, 7) is inside a red square.
...
begin
	draw(shield, 
		...
		show_grid=true,
		legend=:topright)
	plot!(#title="$(mechanics), \n G=$G, upto_t=$upto_t, resolution=$resolution", 
			titlefontsize=7,
			xlabel="v", 
			ylabel="p")
end

# ╔═╡ 70716f46-1ef3-4e57-b30b-14a92c7e7597
# shield, _terminated_early_, _animation_
optimisticshield, _, _ = make_shield(R_optimistic, grid)

# ╔═╡ 660593bc-b06a-4aa9-ab07-32c2f4f82470
begin
		draw(optimisticshield, 
			colors=shieldcolors, 
			color_labels=shieldlabels,
			show_grid=true,
			legend=:topright)
...
# ╔═╡ 36619be2-6025-4d62-b8e4-49c3fc0157e2
mergedshield = call() do
	result = Grid(gridconfig...)
	
	for square in shield
		
		square′ = Square(optimisticshield, 
			square.iv, 
			square.ip)
		...
		if get_value(square′) == BAD_SQUARE
			set_value!(square″, VERY_BAD_SQUARE)
		else
			set_value!(square″, get_value(square))
		end
	end
...
# ╔═╡ eaf9e88e-4f54-4aa3-9a95-d24391af47f8
begin
		draw(mergedshield, 
			colors=mergedshieldcolors, 
			color_labels=mergedshieldlabels,
			show_grid=true,
			legend=:topright)
		p1 = plot!(#title="$(mechanics), \n G=$G, upto_t=$upto_t, resolution=$resolution", 
				titlefontsize=7,
				xlabel="v", 
				ylabel="p")
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "b1fa1bfed01b0f63daedb38520be90b74594e556"
