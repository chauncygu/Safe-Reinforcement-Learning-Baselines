...
This leads to the following system definition, in no particular order:

![a bunch of equations. sorry I used a screenshot.](https://i.imgur.com/vODVap2.png)


I've re-written it to something I find easier to read. In particular, I cannot understand why $A_2$ includes mentions of $d_{aux}$. It appears that $A_2$ is where the former pages diverges from the latter.
...
# ╠═╡ skip_as_script = true
#=╠═╡
mechanics = DCMechanics()
  ╠═╡ =#

# ╔═╡ 1f052f6e-5676-4f66-aa9b-a3514bc20525
# Our action space.
# I also considered having off=2 and on=1 like in the paper, but no.
...
function plot_trace!(trace::DCTrace; show_actions=true, params...)
	...
	plot!(elapsed, x2s,
		line=(colors.SUNFLOWER, 3),;
		label="voltage",
		params...)

	if show_actions
		scatter!([(t, x) for (t, x, a) in zip(elapsed, x1s, actions)
					if a == (on)],
