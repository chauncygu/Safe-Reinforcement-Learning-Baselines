...
function compile_all_libdcshield(working_dir, lib_source_code_dir)
    ...
    for shield_file in glob("*.shield", working_dir)
        ...
        libdcshield_file = compile_libdcshield(working_dir, shield_file, lib_source_code_dir)
        updated_location = working_dir ⨝ "$(basename(shield_file)).so"
        mv(libdcshield_file, updated_location, force=true)
...
function check_shields(compiled_shields, working_dir, blueprints_dir, uppaal_dir; test=false, just_print_the_commands=false)
    ...
    for libdcshield_file in compiled_shields
        progress_update("Checking shield $(basename(libdcshield_file))")
        shield_dir = working_dir ⨝ just_shield_description(libdcshield_file)
    ...
    for shield_dir in shield_dirs
        ...
        model = shield_dir ⨝ "DC__PreShielded.xml"
...
function check_safety_of_preshielded(;shields_dir, results_dir, lib_source_code_dir, blueprints_dir, uppaal_dir, test, just_print_the_commands=false)
    compiled_shields, skipped_shields = compile_all_libdcshield(shields_dir, lib_source_code_dir)
...
if abspath(PROGRAM_FILE) == @__FILE__
    #compiled_shields = compile_all_libdcshield(homedir() ⨝ "Results/tab-DCSynthesis/Exported Strategies/", homedir() ⨝ "ReproducibilityPackage/Shared Code/libdcshield")

    #compiled_shields = [homedir() ⨝ "Results/tab-DCSynthesis/Exported Strategies/1 Samples 0.01 G.shield.so",
    #                    homedir() ⨝ "Results/tab-DCSynthesis/Exported Strategies/1 Samples 0.02 G.shield.so",
    #                    homedir() ⨝ "Results/tab-DCSynthesis/Exported Strategies/1 Samples 0.05 G.shield.so",
    #                    homedir() ⨝ "Results/tab-DCSynthesis/Exported Strategies/4 Samples 0.01 G.shield.so"]

    #shield_dirs = check_shields(compiled_shields, homedir() ⨝ "Results/tab-DCSynthesis/Evaluations", "./tab-DCSynthesis/Blueprints", homedir() ⨝ "opt/uppaal-4.1.20-stratego-10-linux64/", test=true, just_print_the_commands=false)

    # shield_dirs = [homedir() ⨝ "Results/tab-DCSynthesis/Evaluations/1 Samples 0.01 G",
    #                homedir() ⨝ "Results/tab-DCSynthesis/Evaluations/1 Samples 0.02 G",
    #                homedir() ⨝ "Results/tab-DCSynthesis/Evaluations/1 Samples 0.05 G",
    #                homedir() ⨝ "Results/tab-DCSynthesis/Evaluations/4 Samples 0.01 G"]

    #read_query_results(shield_dirs, homedir() ⨝ "Results/tab-DCSynthesis/Evaluations/Evaluations.csv")

    shields_dir = homedir() ⨝ "Results/tab-DCSynthesis/Exported Strategies"
    results_dir = homedir() ⨝ "Results/tab-DCSynthesis/Evaluations"
    lib_source_code_dir = homedir() ⨝ "A1/ReproducibilityPackage/Shared Code/libdcshield"
    blueprints_dir = "./tab-DCSynthesis/Blueprints"
