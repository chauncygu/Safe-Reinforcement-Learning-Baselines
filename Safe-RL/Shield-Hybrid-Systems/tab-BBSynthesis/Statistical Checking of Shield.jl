...
# ╔═╡ 8c926973-d3a3-499b-b358-52bd6fba65cf
...
# ╔═╡ 5fe4cf48-0080-4814-899b-aa3d6290f00e
# ╠═╡ skip_as_script = true
#=╠═╡
begin
	colors = 
...
# ╔═╡ 85d9d6fa-7ffd-4c8d-b093-48f5fb18e35f
# ╠═╡ skip_as_script = true
#=╠═╡
begin
...
# ╔═╡ a45ba84f-fb31-471f-bd72-2c5d494d5eee
md"""
## (Action Required) Import the shield
...
# ╔═╡ 4eac0409-078e-4e61-b98c-1ea84a639985
function create_random_policy(hit_chance)
	return (v, p) -> 
...
# ╔═╡ bbc2c035-c18e-42e4-acb0-f35813bec7fb
create_random_policy(0.5)(-3, 10) # Arbitrarily choosing v=-3, p=10

# ╔═╡ 73b6c4b2-dc6b-4a8e-a87f-ed44d262d063
...
# ╔═╡ 0470fb73-aa16-4249-918d-f896cc69efb8
evaluate_safety(mechanics, create_random_policy(1/5), 1000, 
				run_duration=120, min_v_on_impact=1)

# ╔═╡ 3e4e8de4-1010-409a-af03-dceb32c476ac
# ╠═╡ skip_as_script = true
#=╠═╡
call(() -> begin
	hit_chances = [1/i for i in 2:10]
	safety_violation_chance = [
		safety_violations_observed/runs
		for (safety_violations_observed, runs, _) in 
			[
				evaluate_safety(mechanics, create_random_policy(hit_chance), 1000)
				for hit_chance in hit_chances ]]
	percent_safe = [ (1 - x)*100 for x in safety_violation_chance ]
	plot(hit_chances, percent_safe,
		xticks=hit_chances,
		xrotation=-90,
		xscale=:identity,
		markersize=4,
		...
		xlabel="hit chance", ylabel="percentage of runs that are safe",
		legend=nothing,
		size=(800, 700))
end)
  ╠═╡ =#
...
function test_policies(mechanics, policy_functions, policy_labels, number_of_runs)
	result = []
	for (policy, policy_label) in zip(policy_functions, policy_labels)
		...
		safety_violations_observed, _, unsafe_trace =
			evaluate_safety(mechanics, policy, number_of_runs, 
							run_duration=120, 
							min_v_on_impact=1)
...
# ╔═╡ 851968e3-9a17-473b-b5e6-105e864b2e30
call(() -> begin
	random_policies = [ create_random_policy(1/i) for i in 1:10 ]
	policy_labels = [ "hit chance=(1/$i)" for i in 1:10 ]
	test_policies(mechanics, random_policies, policy_labels, 100)
end)

# ╔═╡ b1dad7c2-e01d-411b-8fcb-1293e9b81c0d
...
begin
	"""
		test_shield(mechanics, shield, random_agents_hit_chances, number_of_runs)
	
		
		test_shield(path_to_shield::AbstractString, 
			mechanics,
			random_agents_hit_chances::Number,
			number_of_runs::Number)

	Test shield on random agents with different "hit" frequency.
	"""
	function test_shield(shield::Grid, mechanics, 
		...
		for hit_chance in random_agents_hit_chances
			random_policy = create_random_policy(hit_chance)
			shielded_policy = (v, p) -> shield_action(shield, v, p, random_policy(v, p))
			push!(policy_functions, shielded_policy)
			push!(policy_labels, 
				"shielded random agent with hit chance=($hit_chance)")
		end
		
		result = test_policies(mechanics, policy_functions, policy_labels, number_of_runs)
	...
	function test_shield(path_to_shield::AbstractString, 
		...
		return test_shield(shield, mechanics, random_agents_hit_chances, number_of_runs)
	end
end
...
#=╠═╡
if shield != nothing
	test_shield(shield, mechanics, [0.05], 100)
end
  ╠═╡ =#
...
# ╠═╡ skip_as_script = true
#=╠═╡
test_shield("Export/16-samples.shield", mechanics, [1/i for i in 2:5], 10)
  ╠═╡ =#
...
Load shields from `shields_dir` and test them on random agents with different "hit" frequency. 
...
function test_shields_and_save_results(shields_dir, 
	...
	for path_to_shield in glob("*.shield", shields_dir)
		...
		output = test_shield(path_to_shield, mechanics, 
			random_agents_hit_chances, runs_per_shield)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "3576110b83cfae0d0c1efe593d1b9e8fe8db7adf"
