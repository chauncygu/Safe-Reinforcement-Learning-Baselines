...
`selected_file =` $(@bind selected_file PlutoUI.FilePicker([MIME("text/csv")]))
"""
...
selected_file′ = @nbparam("selected_file", selected_file)

# ╔═╡ bba93717-6370-417d-8b12-01d006623c6a
rawdata = begin
	if typeof(selected_file′) <: AbstractDict
		CSV.read(selected_file["data"], normalizenames=true, DataFrame)
	elseif typeof(selected_file′) <: AbstractString
		open(selected_file′, "r") do data
			CSV.read(data, normalizenames=true, DataFrame)
		end
	else
		throw(error("Unexpected type of selected_file: $(typeof(selected_file′))"))
	end
end
...
function experiment_order(a)
	...
	if length(result) != 1
		throw(error("Undefined experiment name: $a"))
	end
...
begin
	# See "note on BB time-locks"
	bad_rows = filter([:Experiment, :Deterrence] => 
		(e, d) -> ((e == "NoShield" && d == "-") || (e == "PostShielded" && d == "-")),
		rawdata)
	...
	if nrow(bad_rows) > 0
		Markdown.parse("""
!!! danger "Bad row"
	Bad row detected. Probably due to failed queries, as explained here:
...
begin
	safety_violations, shielded_configurations_count = call() do
		shielded = filter(:Experiment => e -> e ∉ ["NoShield"] ,
			rawdata)
	
		safety_violations = filter(:Avg_Deaths => x -> x > 0, shielded)
		if nrow(safety_violations) == 0
			nothing, nrow(shielded)
		else
			safety_violations, nrow(shielded)
		end
	end
	(;safety_violations, shielded_configurations_count)
end
...
call() do
	if safety_violations != nothing
		...
		!!! success "No safety violations :-)"
			...
			No safety violations observed for the dataset's shielded configurations.
...
begin
	# Default values of the check-boxes.
...
`pre_shielded =` $(@bind pre_shielded CheckBox(default=pre_shielded_default))

`post_shielded =` $(@bind post_shielded CheckBox(default=post_shielded_default))

`no_shield =` $(@bind no_shield CheckBox(default=no_shield_default))

`layabout =` $(@bind layabout CheckBox(default=layabout_default))
"""

# ╔═╡ 51c9ff9d-20f3-48fc-819a-d534669ecfd3
mean([0 0 0 0 0 0 100])
...
safety_violations_message = call() do
	...
	if safety_violations != nothing
		...
		("""
		!!! success "No crashes observed for any shielded configuration."
...
Markdown.parse(safety_violations_message)
