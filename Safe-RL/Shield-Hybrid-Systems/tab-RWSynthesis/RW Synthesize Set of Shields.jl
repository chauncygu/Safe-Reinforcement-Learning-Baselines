...
`G =` $(@bind G NumberField(0.01:0.01:5, default=0.005))
"""

# ╔═╡ 868bb66d-79fa-427c-ac43-2033e3324d07
grid = Grid(G, 0, 1.2, 0, 1.2)

# ╔═╡ 8c218256-7c51-4073-8ca5-dfc2c655da85
...
begin
	initialize!(grid)
	shield, _ = make_shield(rwmechanics..., grid)
end
  ╠═╡ =#
...
function make_and_save_shield(G, save_path)
...
# ╠═╡ skip_as_script = true
#=╠═╡
shield_report = make_and_save_shield(0.1, test_dir)
  ╠═╡ =#

# ╔═╡ 6cd2dced-fab7-4ef0-a72c-8cb58d773935
#=╠═╡
draw_shield(robust_grid_deserialization(joinpath(test_dir,  shield_report.saved_as)))
  ╠═╡ =#

# ╔═╡ 84351342-3c6a-491a-bde3-ef9a90b7ef14
function make_and_save_shields(Gs, save_path)
	if !isdir(save_path)
	...
	results = []
	for G in Gs
		result = make_and_save_shield(G, save_path)
		push!(results, result)
		
		progress_update("Shield $(length(results)) done. Valid:$(result.valid). Saved as: $(result.saved_as)")
	end

	report_path = get_unique_file(save_path, "Shields Synthesis Report.csv")
...
# ╠═╡ skip_as_script = true
#=╠═╡
make_and_save_shields([0.01, 0.005], test_dir)
  ╠═╡ =#
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "82877a1f639f997aae213648415b447c11efff0b"
