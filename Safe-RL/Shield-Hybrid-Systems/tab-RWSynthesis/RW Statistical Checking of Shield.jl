...
# ╔═╡ 545c2312-bb87-472f-9e45-fc020d8b4a78
begin
	colors = 
...
# ╔═╡ 51270796-501b-4ca3-b1ec-760941fe5ec4
function draw_shield(shield)
	draw(shield, colors=rwshieldcolors, color_labels=rwshieldlabels)
	plot!(xlabel="x", ylabel="t", legend=:outerright)
end
...
# ╔═╡ 1741d306-d0d3-43d6-a2b0-a56bb8f39527
function shield_is_valid(shield)
	# Shield is invalid if the start state is considered unsafe.
	square = box(shield, 0, 0)
	if get_value(square) == BAD_SQUARE
		return false
	end
...
# ╔═╡ 299bc574-c95b-4a62-b1b5-277fa450faf3
function create_random_policy(fast_chance)
	return (v, p) -> 
		if rand(0:eps():1) <= fast_chance
			:fast
		else
			:slow
		end
...
# ╔═╡ 598e6c73-3eee-4f1e-b238-038da65cb98b
...
# ╔═╡ 6790613e-0871-4c6c-a3c3-f0d6a13ceefd
...
# ╔═╡ 15b0adda-9aaa-491c-91fd-2d5cce61c99e
...
# ╔═╡ 3726f0fd-796c-46ef-9da4-2db35c248c5a
md"""
### Costs
...
(0 by default since we count safety violations seperately.)

cost\_loss = $(@bind cost_loss NumberField(0:1:10000, default=0))
"""

# ╔═╡ ecf5d432-2add-4bec-aebf-519e332a2c02
create_random_policy(1/2)(0.1, 0.1) # x and t don't matter

# ╔═╡ 2d6723c0-d321-406f-84d4-d1ac36ba8c04
function evaluate_safety(policy, number_of_runs;
							unlucky=false)
	...
	for run in 1:number_of_runs
		...
		# Take a walk. I mean simulate a run.
		xs, ts, actions, total_cost, winner = take_walk(cost_function, cost_loss, 
			x_lim, t_lim,
			rwmechanics..., 
			policy, 
			unlucky=unlucky)
		...
		if !winner
			...
			unsafe_trace = (;xs, ts, actions)
		end
	end
	(;safety_violations_observed, number_of_runs, unsafe_trace)
end

# ╔═╡ 21e2980e-e927-43ee-8888-cdc709f14873
take_walk(cost_function, cost_loss, 
	1.0, 1.0, 
	rwmechanics..., 
	create_random_policy(1/2))

# ╔═╡ 81a20cbc-0dc8-451a-8a5e-cf4453ff8fd0
rwmechanics

# ╔═╡ fb3f6dbe-d2aa-4feb-aec7-2975519f4cb8
evaluate_safety(create_random_policy(1/2), 1000)

# ╔═╡ 5c569a1b-f697-4b0e-a5ba-31f6ea838835
call(() -> begin
	fast_chances = [1/i for i in 2:10]
	safety_violation_chance = [
		safety_violations_observed/runs
		for (safety_violations_observed, runs, _) in 
			[
				evaluate_safety(create_random_policy(fast_chance), 1000)
				for fast_chance in fast_chances ]]
	percent_safe = [ (1 - x)*100 for x in safety_violation_chance ]
	plot(fast_chances, percent_safe,
		xticks=fast_chances,
		xrotation=-90,
		xscale=:identity,
		markersize=4,
		...
		title="Safety of unshielded random agents",
		xlabel="Chance of choosing 'fast'", ylabel="percentage of runs that are safe",
		legend=nothing,
		size=(800, 700))
end)

# ╔═╡ f0a8c080-6801-4383-a89a-aa0db2b47d8e
function test_policies(policy_functions, policy_labels, number_of_runs)
	result = []
	for (policy_function, policy_label) in zip(policy_functions, policy_labels)
		# Evaluate the safety of a policy
		safety_violations_observed, _, unsafe_trace =
			evaluate_safety(policy_function, number_of_runs)
		push!(result, (;policy_label, safety_violations_observed, number_of_runs, unsafe_trace))
	end	
	result
end

# ╔═╡ 4963fa26-a2c6-4ea8-96d5-6a1767e5dfbf
call(() -> begin
	random_policies = [ create_random_policy(1/i) for i in 1:10 ]
	policy_labels = [ "fast chance=(1/$i)" for i in 1:10 ]
	test_policies(random_policies, policy_labels, 100)
end)

# ╔═╡ 458b9c0f-3b85-4f55-be37-14382fbc89ca
#=╠═╡
shield_is_valid(shield)
  ╠═╡ =#

# ╔═╡ 6199df6e-31fa-4d74-be44-a6bdda79735f
begin
	"""
		test_shield(mechanics, shield, random_agents_fast_chances, number_of_runs)
	
		
		test_shield(path_to_shield::AbstractString, 
			mechanics,
			random_agents_fast_chances::Number,
			number_of_runs::Number)

	Test shield on random agents with different "fast" frequency.
	"""
function test_shield(shield::Grid,
			random_agents_fast_chances, 
			number_of_runs::Number)
		...
		for fast_chance in random_agents_fast_chances
			random_policy = create_random_policy(fast_chance)
			shielded_policy = get_shielding_function(shield, random_policy)
			push!(policy_functions, shielded_policy)
			push!(policy_labels, 
				"shielded random agent with fast chance=($fast_chance)")
		end
		
		result = test_policies(policy_functions, policy_labels, number_of_runs)
	...
	function test_shield(path_to_shield::AbstractString, 
							random_agents_fast_chances,
							number_of_runs::Number)
		...
		return test_shield(shield, random_agents_fast_chances, number_of_runs)
	end
end
...
"""
	test_multiple_shields_and_save_results(shields_dir, results_dir, 
		random_agents_fast_chances, runs_per_shield)

Load shields from `shields_dir` and test them on random agents with different "fast" frequency. 
...
"""
function test_shields_and_save_results(shields_dir, 
				results_dir, 
				random_agents_fast_chances, 
				runs_per_shield)
	result = []
	report_path = get_unique_file(results_dir, "Test of Shields.csv")
	for path_to_shield in glob("*.shield", shields_dir)
		name = basename(path_to_shield)
		progress_update("Checking shield: $name")
		
		output = test_shield(path_to_shield, 
			random_agents_fast_chances, runs_per_shield)
		...
		# Gotta save the progress every iteration. 
		csv_data = [
		...
		CSV.write(report_path, csv_data)
	end
	result
end
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "22ce0963dc38b8e26ef4084c15c288bdc5da7c98"
