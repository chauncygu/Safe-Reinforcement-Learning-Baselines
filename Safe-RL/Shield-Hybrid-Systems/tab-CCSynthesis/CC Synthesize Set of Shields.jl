...
# ╔═╡ 8d072e94-446c-4342-a3c5-cbd702311b66
shieldcolors=[colors.POMEGRANATE, colors.PETER_RIVER, colors.CONCRETE, colors.CLOUDS]

# ╔═╡ 8787fae5-06e3-497a-8f27-0bb44ca50ca2
shieldlabels=["No actions", "1 Action", "2 Actions", "All 3 Actions"]
...
# ╔═╡ b0d8a84d-2dc3-4c02-80e9-4ebd78c5925d
init_func(lower, upper) = lower[3] <= 0 ? 0 : actions_to_int(CCAction, instances(CCAction))

# ╔═╡ 6a6680c2-4945-4276-bfaa-b8e9befc5862
md"""
### Configure generation of example shield

`G =` $(@bind G NumberField(0.01:0.01:5, default=1))

`samples_per_axis =` $(@bind samples_per_axis NumberField(1:10, default=1))
"""

# ╔═╡ 868bb66d-79fa-427c-ac43-2033e3324d07
grid = get_cc_grid(G, ccmechanics)

# ╔═╡ 76d3a8dc-8e00-47f0-b459-22bf6a86dccf
md"""
`v_ego_to_show =` $(@bind v_ego_to_show NumberField(ccmechanics.v_ego_min:G:ccmechanics.v_ego_max, default=0))
"""

# ╔═╡ 2b7c8520-3f1e-4091-b4cc-233fb5dfc19e
# Write as R\hat
R̂ = get_barbaric_reachability_function(samples_per_axis, ccmechanics)

# ╔═╡ 218a380e-6be4-4e08-a6ea-f1d5c16a6e8d
R̂(box(grid, [0, 0, 10]), forwards)
...
begin
	...
	shield, _ = make_shield(R̂, CCAction, grid)
end
  ╠═╡ =#
...
# ╔═╡ 85f4335a-e969-476b-a6c7-732c86961144
...
#=╠═╡
draw_shield(shield, CCAction, v_ego=v_ego_to_show)
  ╠═╡ =#
...
function make_and_save_barbaric_shield(samples_per_axis, G, save_path)
	...
	# Save shield
	sample_count = length(possible_outcomes(samples_per_axis, ccmechanics, 
			first(shield), neutral))
	file_name = "CC $sample_count Samples $(grid.G) G.shield"
	file_path = get_unique_file(save_path, file_name)
	robust_grid_serialization(file_path, shield)
	plot = draw_shield(shield, CCAction)
	savefig(plot, file_path * ".png")

	return (saved_as=basename(file_path), 
		samples_per_axis, 
		gridargs=(shield.G, shield.lower_bounds, shield.upper_bounds),
...
# ╠═╡ skip_as_script = true
#=╠═╡
barbaric_shield = make_and_save_barbaric_shield(1, 1, test_dir)
  ╠═╡ =#

# ╔═╡ 6cd2dced-fab7-4ef0-a72c-8cb58d773935
#=╠═╡
draw_shield(
	robust_grid_deserialization(joinpath(test_dir,  barbaric_shield.saved_as)), 
	CCAction,
	title=barbaric_shield.saved_as)
  ╠═╡ =#

# ╔═╡ 1ea060e2-d10c-454a-b6e3-85f54796c793
function estimate_time(samples_per_axis::Number, G::Number)
	...
	samples = samples_per_axis^3*5 # 3 axes and 5 possible actions

	# Number of squares
	lower, upper = 
		[ccmechanics.v_ego_min, 
		ccmechanics.v_front_min, 
		-1],
		[ccmechanics.v_ego_max + 1, 
		 ccmechanics.v_front_max + 1, 
		 ccmechanics.ego_sensor_range + 2]

	
	grid_size = zeros(Int, 3)
	for (i, (lb, ub)) in enumerate(zip(lower, upper))
		grid_size[i] = ceil((ub-lb)/G)
	end
...
function estimate_time(samples_per_axiss::Vector, Gs::Vector)
...
function make_and_save_barbaric_shields(samples_per_axiss, Gs, save_path)
	...
	estimated_seconds = estimate_time(samples_per_axiss, Gs)
...
# ╠═╡ skip_as_script = true
#=╠═╡
make_and_save_barbaric_shields([1], [1], 
	test_dir)
  ╠═╡ =#

# ╔═╡ aaea7ab8-19ab-4bfd-8e46-03e417328920
# ╠═╡ skip_as_script = true
#=╠═╡
estimate_time(3, 0.5), 2991 + 1377
  ╠═╡ =#

# ╔═╡ 462d5cb0-51bd-4544-8332-c0e37a3b26b2
# ╠═╡ skip_as_script = true
#=╠═╡
estimate_time([3], [0.5])
  ╠═╡ =#

# ╔═╡ 2a7b64f6-c802-4f7c-850d-b2709800c536
estimate_time([3, 5], [1, 0.5])/60/60
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "480d40c7cba5cc1aefc6bbd471fead21af0bdb5e"
