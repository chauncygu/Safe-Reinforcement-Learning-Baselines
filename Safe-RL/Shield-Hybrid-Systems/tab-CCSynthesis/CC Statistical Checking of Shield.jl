...
!!! danger "Please select file"
	...
	draw_shield(shield, CCAction, v_ego=0)
end
  ╠═╡ =#
...
call() do 
	...
	random_fallback(_, allowed_actions) = sample([allowed_actions...])
	shielding_function′ =  get_shielding_function(shield, CCAction, random_fallback)
	
	random_behaviour(_) = sample([instances(CCAction)...])
	random_shielded(s) = shielding_function′(s, random_behaviour(s))
	random_front_behaviour = get_random_front_behaviour(ccmechanics)
	
	anim = @animate for i in 1:30
		sequence = simulate_sequence(ccmechanics, random_front_behaviour, 
			120, (0., 0., 100.), random_shielded)
		plot_sequence(sequence...)
	end
	gif(anim, joinpath(tempdir(), "gif3.gif"), fps=1)
end
  ╠═╡ =#

# ╔═╡ 73b6c4b2-dc6b-4a8e-a87f-ed44d262d063
function evaluate_safety(policy, number_of_runs; run_duration=120)
	random_front_behaviour = get_random_front_behaviour(ccmechanics)
	...
	for run in 1:number_of_runs
		...
		states, times = simulate_sequence(ccmechanics, random_front_behaviour, 
						run_duration, s0, policy)
		# See if at any point distance < 0
		if any([s[3] <= 0 for s in states])
			safety_violations_observed += 1
			unsafe_trace = (;states, times)
		end
	end
...
call() do 
	random_behaviour(_) = sample([instances(CCAction)...])
	evaluate_safety(random_behaviour, 1000, 
				run_duration=120)
end
...
function test_policies(policy_functions, policy_labels, number_of_runs)	
	...
	for (policy, policy_label) in zip(policy_functions, policy_labels)
		...
		safety_violations_observed, _, unsafe_trace =
			evaluate_safety(policy, number_of_runs, run_duration=120)
...
begin
	...
	"""
	function test_shield(shield::Grid, number_of_runs::Number)
		...
		policy_functions = [random_shielded]
	...
	function test_shield(path_to_shield::AbstractString, number_of_runs::Number)
		...
		return test_shield(shield, number_of_runs)
	end
end
...
# ╠═╡ skip_as_script = true
#=╠═╡
test_shield("Export/CC 8 samples with G of 1.0.shield", 10)
  ╠═╡ =#
...
"""
function test_shields_and_save_results(shields_dir, results_dir, runs_per_shield)
	result = []
	for path_to_shield in glob("*.shield", shields_dir)
		name = basename(path_to_shield)
		progress_update("Checking shield: $name")
		
		output = test_shield(path_to_shield, runs_per_shield)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "0fd275ba224b9a593414dc0f41c2c3bddc940b01"
