...
begin
	...
	include("../Shared Code/BBRigorousReachabilityFunction.jl")
...
# ╔═╡ 0ae0c4c8-fbed-466d-b965-b0c53d82d115
begin
	HIT_NOHIT = 0 	# {hit, nohit}
	HIT_ONLY = 1  	# {hit} 
	BAD = 2			# {}
end

# ╔═╡ d0d9d7d9-5349-46de-b89e-66c5ea5aeffd
md"""
### Color shceme
...
# ╔═╡ 63964e5d-35a1-4eef-be40-cf5b3226574f
# ╠═╡ skip_as_script = true
#=╠═╡
begin
	colors = 
...
# ╔═╡ 80723e88-b663-46eb-89a7-60368329386d
# ╠═╡ skip_as_script = true
#=╠═╡
begin
...
# ╔═╡ 5c42ba02-c5f0-4d4c-8203-cc2d1fbc9b8e
@bind mechanics PlutoUI.combine() do Child
md"""
#### Configure Mechanics of the Ball
...
# ╔═╡ 2c4dddd7-451a-407b-9912-8f1e70287d48
begin
	@bind gridconfig confirm(PlutoUI.combine() do Child
md"""
### Configure Grid

Set `G=0.02` to generate a full shield. (Takes up to 3 minuts 20 seconds)
...
# ╔═╡ 7e6b88f5-93f3-4811-a535-19c6b6fcc86d
md"""
`samples_per_axis =` $(@bind samples_per_axis confirm(NumberField(1:30, default=4)))
"""
...
# ╔═╡ 267d79dc-58f5-48ff-9963-4339c6920e5b
begin
	grid = Grid(gridconfig...)
	grid
end

# ╔═╡ 2b7c8520-3f1e-4091-b4cc-233fb5dfc19e
# Write as R\hat
R̂ = get_barbaric_reachability_function(samples_per_axis, mechanics)

# ╔═╡ 218a380e-6be4-4e08-a6ea-f1d5c16a6e8d
R̂(box(grid, 1, 2), "nohit")

# ╔═╡ b17a76bb-f4cf-4f35-938a-ecc66448245b
# Function used for initializing the squares
function init_function(Ivl, Ivu, Ipl, Ipu)
	if Ipl == 0 && abs(Ivl) <= 1
		BAD
	else
		HIT_NOHIT
...
#=╠═╡
begin
	initialize!(grid, init_function)
	shield, _, _ = make_shield(R̂, grid, max_steps=typemax(Int))
end
  ╠═╡ =#

# ╔═╡ 18909903-0470-41c8-9221-42d18b0513de
#=╠═╡
draw(shield, colors=shieldcolors, color_labels=shieldlabels)
  ╠═╡ =#
...
function make_and_save_barbaric_shield(samples_per_axis, gridargs, save_path)
	...
	_, seconds_taken, bytes_used, gctime, gcstats = @timed begin
		shield, _, _ = make_shield(R̂, grid, max_steps=typemax(Int), animate=false)
	end

	# Save shield
	sample_count = length(grid_points(first(shield), samples_per_axis))
	file_name = "$sample_count Samples $(grid.G) G.shield"
	file_path = get_unique_file(save_path, file_name)
	robust_grid_serialization(file_path, shield)
	
	return (saved_as=basename(file_path), samples_per_axis, gridargs, 
		valid=shield_is_valid(shield), seconds_taken, 
		bytes_used, gctime, 
...
# ╔═╡ 4216dd2b-0124-4a58-8e86-ae14ee6e355e
function make_and_save_rigorous_shield(algorithm, alg_label, gridargs, save_path)
	# Prerequesites for synthesizing shield
	R = get_rigorous_reachability_function(mechanics, algorithm)
	grid = Grid(gridargs...)
	initialize!(grid, init_function)
	
	# Mainmatter
	_, seconds_taken, bytes_used, gctime, gcstats = @timed begin
		shield, _, _ = make_shield(R, grid, max_steps=typemax(Int), animate=false)
	end

	# Save shield
	file_name = "$alg_label with G of $(grid.G).shield"
	file_path = get_unique_file(save_path, file_name)
	robust_grid_serialization(file_path, shield)
	plot = draw(shield, 
		colors=[colorant"#ffffff", colorant"#a1eaff", colorant"#ff9178"])
	savefig(plot, file_path * ".png")

	return (saved_as=basename(file_path), alg_label, gridargs, 
		valid=shield_is_valid(shield), seconds_taken, 
		bytes_used, gctime, 
		gcstats.allocd, gcstats.malloc, gcstats.realloc, gcstats.poolalloc, gcstats.bigalloc, gcstats.freecall, gcstats.total_time, gcstats.pause, gcstats.full_sweep)
end
...
# ╔═╡ ae53934c-842a-46c2-a32f-9da54af0b305
#=╠═╡
rigorous_shield = make_and_save_rigorous_shield(BOX(δ=0.02), "BOX0.02", (0.1, -13, 13, 0, 8), test_dir)
  ╠═╡ =#

# ╔═╡ 204bc80d-a0f5-4ffb-902e-731901e5cc35
#=╠═╡
draw(robust_grid_deserialization(joinpath(test_dir, rigorous_shield.saved_as)), 
	colors=shieldcolors, color_labels=shieldlabels,
	title=rigorous_shield.saved_as)
  ╠═╡ =#

# ╔═╡ 451aa409-aff6-4104-8108-44600971fad0
# ╠═╡ skip_as_script = true
#=╠═╡
barbaric_shield = make_and_save_barbaric_shield(10, (0.1, -13, 13, 0, 8), test_dir)
  ╠═╡ =#

# ╔═╡ 6cd2dced-fab7-4ef0-a72c-8cb58d773935
#=╠═╡
draw(robust_grid_deserialization(joinpath(test_dir, barbaric_shield.saved_as)), 
	colors=shieldcolors, color_labels=shieldlabels,
	title=barbaric_shield.saved_as)
  ╠═╡ =#

# ╔═╡ 84351342-3c6a-491a-bde3-ef9a90b7ef14
function make_and_save_barbaric_shields(samples_per_axiss, gridargss, save_path)
	if !isdir(save_path)
	...
	nshields = length(samples_per_axiss)*length(gridargss)
	progress_update("The following number of shields will be synthesized: $nshields")
	
	results = []
	for samples_per_axis in samples_per_axiss
		for gridargs in gridargss
			result = make_and_save_barbaric_shield(samples_per_axis, gridargs, save_path)
			push!(results, result)
...
# ╠═╡ skip_as_script = true
#=╠═╡
make_and_save_barbaric_shields([2, 4, 8], [(0.1, -13, 13, 0, 8)], 
	test_dir)
  ╠═╡ =#

# ╔═╡ 796d24c1-1b7d-40f3-9e13-93ce5aba5cff
algorithms = [
		AlgorithmInfo(BOX(δ=0.01), 4, 
			"BOX 0.01"),
		AlgorithmInfo(BOX(δ=0.002), 20, 
			"BOX 0.002"),
		AlgorithmInfo(GLGM06(δ=0.1, max_order=10, approx_model=Forward()), 4, 
			"GLGM06 0.1"), 
...
function estimate_time(algorithms::Vector{AlgorithmInfo}, gridargss)
	ms = 1/1000
	time = 0
	for gridargs in gridargss
		grid = Grid(gridargs...)
		...
		for algorithm_info in algorithms
			time += grid_length * algorithm_info.milliseconds_per_call * ms
		end
	end
	return time
end

# ╔═╡ fe0b6fc3-2c00-4577-a920-efef4c7d0314
function make_and_save_rigorous_shields(algorithms::Vector{AlgorithmInfo}, gridargss, save_path)
	if !isdir(save_path)
		throw(ArgumentError("Not a directory: $save_path"))
	end

	report_path = get_unique_file(save_path, "Rigorous Shields Synthesis Report.csv")

	nshields = length(algorithms)*length(gridargss)
	progress_update("The following number of shields will be synthesized: $nshields")
	time_estimate = estimate_time(algorithms, gridargss)
	progress_update("Estimated time: $time_estimate seconds.")
	
	results = []
	for algorithm_info in algorithms
		algorithm, alg_label = algorithm_info.algorithm, algorithm_info.label
		for gridargs in gridargss
			result = make_and_save_rigorous_shield(algorithm, alg_label, gridargs, save_path)
			push!(results, result)
...
# ╔═╡ c2d3008f-f3d7-43ff-881e-bfe5b610bc6a
#=╠═╡
make_and_save_rigorous_shields(algorithms, [(1, -13, 13, 0, 8)], test_dir)
  ╠═╡ =#
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "077b97b4aa2704b35c02cd2adfa66c158d590c9e"
...
[[deps.CarlemanLinearization]]
deps = ["IterTools", "LinearAlgebra", "MultivariatePolynomials", "Requires", "SparseArrays"]
git-tree-sha1 = "9f4c67995ff518913ccae123b233af975579f481"
...
[[deps.HybridSystems]]
deps = ["FillArrays", "Graphs", "LinearAlgebra", "MappedArrays", "MathematicalSystems"]
git-tree-sha1 = "9d9384300bd8e1352667b5ce0f1a85630b4b90ca"
...
git-tree-sha1 = "da2424e384515415f7180d84b9ff40e2463532fb"
...
deps = ["CarlemanLinearization", "CommonSolve", "ExprTools", "HybridSystems", "IntervalArithmetic", "IntervalMatrices", "LazySets", "LinearAlgebra", "MathematicalSystems", "Parameters", "ReachabilityBase", "RecipesBase", "RecursiveArrayTools", "Reexport", "Requires", "SparseArrays", "StaticArrays", "TaylorIntegration", "TaylorModels", "TaylorSeries"]
git-tree-sha1 = "01628ccd44383fc8d8874814498bf37d4fb67e1d"
...
git-tree-sha1 = "ff7335089fea792c22dc7eb37895f5de7b4522c5"
...
[[deps.TaylorIntegration]]
deps = ["DiffEqBase", "Elliptic", "Espresso", "InteractiveUtils", "LinearAlgebra", "Logging", "Markdown", "OrdinaryDiffEq", "RecursiveArrayTools", "Reexport", "Requires", "StaticArrays", "TaylorSeries"]
git-tree-sha1 = "216f7847a97d193472a4a0a9920778e0a16124f2"
...
git-tree-sha1 = "45fe465e3a7436a6f69a7e72748ae9ed4c2d41a9"
