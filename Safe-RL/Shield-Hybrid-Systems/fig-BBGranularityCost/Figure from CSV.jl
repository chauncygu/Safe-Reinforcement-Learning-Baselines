...
# ╔═╡ 95a61f84-7a2a-11ed-361c-9d46912c45e3
begin
	using CSV
	using Plots
	using PlutoUI
...
# ╔═╡ 8f04cc23-f9f4-4efb-a668-730fa852aebc
call(f) = f()

# ╔═╡ 6b4c0ff0-39e6-40df-b1a4-2dd3e7742d41
⨝ = joinpath

# ╔═╡ 3046118b-de46-4411-bc0d-3a465df3301c
default_file = @nbparam("csv_file",
	homedir() ⨝ "Results/fig-BBGranularityCost/Query Results/Results.csv")

# ╔═╡ 1116ca75-7a63-4450-b0b7-2bfd967b3b9e
md"""
`csv_file =` $(@bind csv_file TextField(default=default_file))
"""

# ╔═╡ 300fd0e2-8d12-4e26-a4a9-dd91315ded1f
rawdata = CSV.read(csv_file, DataFrame)

# ╔═╡ d4b74fc4-522a-4cf9-bb9f-e55f15293d1e
begin
	function get_granularity(shield_names::AbstractVector)
		get_granularity.(shield_names)
	end
	
	function get_granularity(shield_name::AbstractString)
		m = match(r"([0-9.]+) G", shield_name)
		parse(Float64, m[1])
	end
end
...
cleandata = call() do
	transform(rawdata, :shield_name => get_granularity => :granularity)
end

# ╔═╡ c9d036c3-d56c-46ee-a89a-0e1bdb2ae28a
medians = call() do
	medians = groupby(cleandata, [:shield_name, :runs, :granularity])
...
granularity_cost_plot = call() do
	...
	ylims = (min(cleandata[!, :cost]...) - margin, 
			 max(cleandata[!, :cost]...) + margin)
	
	@df medians plot(:granularity, :cost,
		markershape=:star,
		markerstrokewidth=1,
...
safety_violations = sum(rawdata[!, :deaths])

# ╔═╡ 3c9ab394-7e2f-47f4-a634-3b1d0f9e4b9f
total_runs = sum(rawdata[!, :runs])

# ╔═╡ 49632915-6977-452d-a7dd-4d99d2722b9a
safety_notice = safety_violations == 0 ? """
!!! success "No safety violations"
	No safety violations observed over the course of $total_runs runs.
""" : """
!!! danger "Safety violations"
	Safety violations observed. This should not happen. Check shield generation parameters.

	Observed $safety_violations over the course of $total_runs runs.
"""

# ╔═╡ d7eda9f6-9357-419a-85fe-8dc9afeb85a7
Markdown.parse(safety_notice)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "40ff98ba77bacb44f404753690e021481b95f26b"
